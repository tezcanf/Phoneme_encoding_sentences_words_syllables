...
# -*- coding: utf-8 -*-
"""
Cohort Model: Phoneme Surprisal and Entropy Calculation (Chinese)

This script calculates phoneme surprisal and entropy values for syllables in Chinese stories
...
class PronunciationDictionary:
    ...
    def build_from_text(self, text_data):
        ...
        # Create bidirectional mapping (sorted for consistency)
...
def calculate_initial_phoneme_frequencies(dictionary, df_freq, df_word_grapheme, 
    ...
    Args:
        ...
        df_freq: DataFrame with word frequencies ('Word' and 'W_million' columns)
        ...
        for word in cohort:
            ...
            if phoneme_str not in df_word_grapheme['phoneme'].values:
                ...
                if len(freq_row) == 0:
                    ...
                    freq = float(freq_row.iloc[-1]['W_million'])
...
def get_word_frequency_surprisal(grapheme, df_freq, num_words_all_cohort):
    ...
    if len(freq_row) > 0:
        word_freq = float(freq_row.iloc[-1]['W_million'])
...
def main():
    ...
    dict_filename = 'Chinese_dict.txt'
    freq_filename = 'Chinese_freq_file.xlsx'
    ...
    num_words_all_cohort = 999016  # Total words in corpus
    ...
    # Process story files
    phoneme_files = sorted([f for f in os.listdir(phoneme_folder) if f.endswith('.txt')])
    
    # Process batch (first 70 files)
    file_batch = 1
    phoneme_files = phoneme_files[(file_batch - 1) * 70 : file_batch * 70]
    
    print(f"\nProcessing {len(phoneme_files)} story files...")
    
    for phoneme_file in phoneme_files:
        process_story_file(
            phoneme_file, phoneme_folder, phoneme_words_all, dictionary,
            df_word_grapheme, df_freq, num_words_all_cohort, initial_counter,
            output_folder
        )
