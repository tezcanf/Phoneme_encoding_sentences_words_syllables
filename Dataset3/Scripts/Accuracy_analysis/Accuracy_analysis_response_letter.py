...
# -*- coding: utf-8 -*-
"""
TRF Phoneme Features Analysis: Onset vs Surprisal/Entropy
Compares explained variance (R²) improvements for phoneme onset features
versus phoneme surprisal/entropy features across different experimental conditions.
...
ANALYSES = {
    'sentences_vs_words': {
        ...
        'models': [
            # Phoneme Onset: acoustic+phoneme_onset+words → acoustic+phonemes+words
            ...
            # Phoneme Surp/Ent: acoustic+phoneme_surp_entp+words → acoustic+phonemes+words
        ...
        'sig_signs': ['n.s.', '***', '.', 'n.s.'],
        'output_csv': 'Dataset3_Accuracies_both_hemispheres_sentences_vs_words_STG_Delta_Theta_phoneme_onset_vs_surp_ent_revision.csv',
        'output_fig': 'Phoneme_features_sentences_vs_words.svg'
    },
    'words_vs_syllables': {
        'conditions': ['random_word_list', 'random_syllables'],
        'features': ['Phoneme Onset', 'Phoneme Surprisal/Entropy'],
        'models': [
            # Phoneme Onset: acoustic+phoneme_onset → acoustic+phonemes
            ...
            # Phoneme Surp/Ent: acoustic+phoneme_surp_entp → acoustic+phonemes
        ...
        'sig_signs': ['n.s.', '****', '****', '**'],
        'output_csv': 'Dataset3_Accuracies_both_hemispheres_words_vs_syllables_STG_Delta_Theta_phoneme_onset_vs_surp_ent_revision.csv',
        'output_fig': 'Phoneme_features_words_vs_syllables.svg'
    }
}
...
def calculate_accuracy(subject, condition, models):
    ...
    # Mean across sources for each hemisphere, then mean across hemispheres
    lh_mean = trf_lh_full.proportion_explained.mean(axis='source')
    rh_mean = trf_rh_full.proportion_explained.mean(axis='source')
    
    return np.mean([lh_mean, rh_mean])
...
def run_analysis(analysis_name):
    ...
    ----------
    analysis_name : str
        Name of analysis ('sentences_vs_words' or 'words_vs_syllables')
    """
...
if __name__ == "__main__":
    print("=" * 70)
    print("PHONEME FEATURES ANALYSIS: ONSET VS SURPRISAL/ENTROPY")
    print("=" * 70)
    
    # Run both analyses
    results = {}
    
    # Analysis 1: Sentences vs Words
    results['sentences_vs_words'] = run_analysis('sentences_vs_words')
    
    # Analysis 2: Words vs Syllables
