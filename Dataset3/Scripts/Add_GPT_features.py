# -*- coding: utf-8 -*-
"""
GPT-2 Surprisal and Entropy Calculation for Turkish MEG Word Lists

Calculates word-level surprisal and entropy values using Turkish GPT-2 models
...
DATA_FOLDER =  root / 'Materials' / 'Cohort_model' / 'Word_list'  # Change Word_list with Sentences
OUTPUT_FOLDER = root / 'Materials' / 'Cohort_model' / 'Word_list' /'with_word_features' # Change Word_list with Sentences

# Model selection
MODEL_NAME = "ytu-ce-cosmos/turkish-gpt2-large-750m-instruct-v0.1"

# Language-specific context initialization
CONTEXT_START_WORD = 'başla'  # Turkish for "start"
...
def load_gpt2_model(model_name):
    """
    Load Turkish GPT-2 tokenizer and model.
...
def calculate_surprisal_and_entropy(graph_words, phonemes_words, tokenizer, model, 
                                   context_start=CONTEXT_START_WORD):
    """
    ...
    model : GPT2LMHeadModel
        GPT-2 model
    context_start : str
        Initial context word (default: 'başla' for Turkish)
...
def process_all_files():
    ...
    # Get all CSV files
    phoneme_files = [f for f in os.listdir(DATA_FOLDER) if f.startswith('control_sentence') or f.startswith('random_word_list')]
    ...
    for file_idx, phoneme_file in enumerate(phoneme_files):
        ...
        # Save results
        output_filename = phoneme_file.replace('.csv', '_GPT.csv')
        output_path = os.path.join(OUTPUT_FOLDER, output_filename)
        
        df_all.to_csv(output_path, index=False, sep=';', line_terminator='\n')
        print(f"  Saved: {output_filename}")
    
    print("\nAll files processed successfully! (Bitti!)")
